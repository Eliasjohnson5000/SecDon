{
  ...
  ... }:
  {
  flake.nixosModules.niri = { config, pkgs, lib, userSettings, ... }: { # The Niri module
    programs.niri = {
      enable = true;  #Enables niri in the system
      package = self.packages.${pkgs.stdenv.hostPlatform.system}.myNiri;  #calls the niri wrapper
    };
  };


  perSystem = { pkgs, lib, self', ... }: {
    packages.myNiri = inputs.wrapper-modules.wrappers.niri.wrap {   #Niri wrapper \/
      inherit pkgs;
      settings = {
        spawn-at-startup = [
        ...
        layout = {
            ...
            always-center-single-column = _:{};   #when there is a single window, it will always be centered
            preset-column-widths = [              # preset widths for switch-preset-column-width keybind (Mod+R by defualt)
                {proportion = 0.5; }
                {proportion = 0.66667; }
                {proportion = 1.0; }
            ];
    ...
    # ------------------------------------------------------------ #
          ...
          "Mod+B".spawn-sh    = "${userSettings.webBrowser}";   #launches web browser
          "Mod+S".spawn       = "${userSettings.fileBrowser}";  #launches file browser
          ...
          "Mod+C".spawn-sh = "niri msg pick-color | grep 'Hex:' | cut -d' ' -f2 | wl-copy"; #on click, puts the hex code for the color you clicked on into your clipboard
          ...
          "Mod+Y".spawn-sh = lib.getExe pkgs.fuzzel;  #launches fuzzle


          #TODO: make overide for user keybinds
